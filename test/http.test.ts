...
test("performs a real GET and returns status, headers and body", async () => {
  await withServer(
    ...
    async (baseUrl) => {
      const resp = await nodeHttpTransport({ method: "GET", url: `${baseUrl}/feed.xml?state=bayern` });
      assert.equal(resp.status, 200);
      assert.match(resp.body.toString("utf8"), /state=bayern/);
    },
  );
});

test("rejects an unsupported protocol with LebensmittelwarnungNetworkError", async () => {
...
test("timeoutMs bounds the whole response, not just idle gaps", async () => {
  ...
  await withServer(
    (_req, res) => {
      ...
      const finish = setTimeout(() => res.end("</rss>"), 2000);
      res.on("close", () => {
        clearInterval(drip);
        clearTimeout(finish);
      });
    ...
    async (baseUrl) => {
      ...
      await assert.rejects(
        ...
        (err) => err instanceof LebensmittelwarnungNetworkError && /timed out after 300ms/.test(err.message),
      );
