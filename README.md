...
| `--base-url <url>` | API base URL (default `https://www.lebensmittelwarnung.de`; `http:`/`https:` only, no query `?` or fragment `#`). A `user:password@` in it is sent as Basic auth and shown as `***@` in error messages |
