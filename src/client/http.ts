...
import { LebensmittelwarnungNetworkError, redactUrl } from "./errors.js";
...
export const nodeHttpTransport: Transport = (request) =>
  new Promise<HttpResponse>((resolve, reject) => {
    ...
    if (url.protocol !== "http:" && url.protocol !== "https:") {
      reject(new LebensmittelwarnungNetworkError(`Unsupported protocol "${url.protocol}" in URL: ${redactUrl(request.url)}`));
      return;
    }
        ...
        (res) => {
          ...
          res.on("data", (chunk: Buffer) => {
            ...
            if (maxBytes !== undefined && received > maxBytes) {
              aborted = true;
              res.destroy();
              fail(new LebensmittelwarnungNetworkError(`Response exceeded maxResponseBytes (${maxBytes})`));
              return;
            }
            chunks.push(chunk);
          });
          ...
          res.on("error", (err) => {
            if (aborted) return;
            fail(new LebensmittelwarnungNetworkError(`Response stream error: ${err.message}`, { cause: err }));
          });
        },
      );
    } catch (err) {
      fail(
        new LebensmittelwarnungNetworkError(`Invalid request: ${err instanceof Error ? err.message : String(err)}`, {
          cause: err,
        }),
      );
      return;
    ...
    if (request.timeoutMs && request.timeoutMs > 0) {
      ...
      timer = setTimeout(() => {
        const err = new LebensmittelwarnungNetworkError(`Request timed out after ${timeoutMs}ms`);
        fail(err);
        req.destroy(err);
    ...
    req.on("error", (err) => {
      fail(err instanceof LebensmittelwarnungNetworkError ? err : new LebensmittelwarnungNetworkError(err.message, { cause: err }));
    });
