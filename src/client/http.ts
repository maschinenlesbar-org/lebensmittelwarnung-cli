  ...
  new Promise<HttpResponse>((resolve, reject) => {
      ...
      (res) => {
        ...
        res.on("data", (chunk: Buffer) => {
          ...
          if (maxBytes !== undefined && received > maxBytes) {
            aborted = true;
            res.destroy();
            reject(new LebensmittelwarnungNetworkError(`Response exceeded maxResponseBytes (${maxBytes})`));
            return;
          }
          chunks.push(chunk);
        });
        ...
        res.on("error", (err) => {
          if (aborted) return;
          reject(new LebensmittelwarnungNetworkError(`Response stream error: ${err.message}`, { cause: err }));
        });
      },
    );
    ...
    if (request.timeoutMs && request.timeoutMs > 0) {
      req.setTimeout(request.timeoutMs, () => {
        req.destroy(new LebensmittelwarnungNetworkError(`Request timed out after ${request.timeoutMs}ms`));
      });
    }

    req.on("error", (err) => {
      reject(err instanceof LebensmittelwarnungNetworkError ? err : new LebensmittelwarnungNetworkError(err.message, { cause: err }));
    });
